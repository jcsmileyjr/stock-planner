...
export default function InputStatus ({label, property, data = "scouted"}: {label: string, property: string, data: string}) {
    ...
    return (
        <div className="flex flex-col mb-2">    
            ...
            <select value={value} className="border-2 border-slate-500 rounded-md p-2" name={label} id={label} onChange={(e) => {updateState(e, property, state, dispatch), setValue(e.target.value)}}>
